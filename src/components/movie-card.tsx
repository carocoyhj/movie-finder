import { BookmarkSimple, CalendarBlank, FilmSlate } from "@phosphor-icons/react";
	...
	return (
		<Card
			className={`group overflow-hidden rounded-[1.75rem] py-0 text-white shadow-2xl shadow-black/20 transition-transform duration-300 hover:-translate-y-1 hover:bg-white/10 ${
				isSelected ? "border-rose-300/45 bg-white/12" : "border-white/10 bg-white/8"
			}`}
		>
			<div className="relative aspect-[4/5] overflow-hidden">
				{hasPoster ? (
					...
					<div className="flex h-full w-full flex-col items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-slate-950 text-white/55">
						...
						<p className="mt-4 text-xs uppercase tracking-[0.35em]">No poster</p>
					</div>
				)}
			...
			<CardContent className="space-y-4 p-5">
				<div className="space-y-2">
					<div className="flex items-start justify-between gap-3">
						...
						<Badge variant="secondary" className="rounded-full bg-white/8 text-white hover:bg-white/10">
							{type}
						</Badge>
					</div>
				</div>
